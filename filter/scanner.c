...
UNICODE_STRING myCheckingDir = RTL_CONSTANT_STRING(L"\\test\\");
...
const FLT_OPERATION_REGISTRATION Callbacks[] = {
	{ IRP_MJ_CREATE, 0, ScannerPreCreate, NULL },
...
--*/
{
	...
	if (NT_SUCCESS(status)) {
		InitializeObjectAttributes(&oa,
			&uniString, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, sd);
...
Routine Description:
	This routine sets the the extensions for files to be scanned based on the registry.
...
--*/
{
	...
	String->Buffer = ExAllocatePoolWithTag(NonPagedPool,
		String->MaximumLength, SCANNER_STRING_TAG);
...
--*/
{
	...
	if (String->Buffer) {
		ExFreePoolWithTag(String->Buffer, SCANNER_STRING_TAG);
		String->Buffer = NULL;
	}
...
--*/
{
	...
	// Note, since we limited max connections to 1, another connect
	// will not be allowed until we return from the disconnect routine.
...
--*/
{
	...
	if (IoThreadToProcess(Data->Thread) == ScannerData.UserProcess) {
		DbgPrint("scanner!ScannerPreCreate: allowing create for trusted process");
	...
	if ((createDisposition == FILE_CREATE) ||
		...
		ScannerAllocateUnicodeString(&fileName);
		RtlUnicodeStringCopy(&fileName, &nameInfo->Name);

		if (DfFindSubString(&fileName, &myCheckingDir)) {
			DbgPrint("File has been created:\n\t\t%wZ\n", &nameInfo->Name);
		}

		FltReleaseFileNameInformation(nameInfo);

		ScannerFreeUnicodeString(&fileName);
	}
...
--*/
{
	...
	(VOID)ScannerpScanFileInUserMode(FltObjects->Instance,
		FltObjects->FileObject, &safeToOpen);
	...
	)
{
	...
	try {
		notification = ExAllocatePoolWithTag(NonPagedPool,
			sizeof(SCANNER_NOTIFICATION), 'nacS');
		...
		notification->BytesToScan = (ULONG)fileName->Length;
		// Copy only as much as the buffer can hold.
		RtlCopyMemory(&notification->Contents,
			fileName->Buffer, min(notification->BytesToScan, SCANNER_READ_BUFFER_SIZE));
...
Routine Description:
	...
	is for a directory, and there's no data to read and scan.
...
--*/
{
	...
	*SafeToOpen = TRUE;
	...
	try {
		...
		// Determine sector size. Noncached I/O can only be done at sector size offsets,
		// and in lengths which are multiples of sector size. A more efficient way is
		// to make this call once and remember the sector size in the instance
		// setup routine and setup an instance context where we can cache it.

		status = FltGetVolumeProperties(volume,
			&volumeProps, sizeof(volumeProps), &length);

		// STATUS_BUFFER_OVERFLOW can be returned - however we only need the properties,
		// not the names hence we only check for error status.
		...
		if (NT_SUCCESS(status) && (bytesRead != 0)) {
			...
			// Copy only as much as the buffer can hold.
			RtlCopyMemory(&notification->Contents,
				buffer, min(notification->BytesToScan, SCANNER_READ_BUFFER_SIZE));
...
{
	INT index;
	INT subStrLen = SubStr->Length / sizeof(WCHAR);
	INT strLen = Str->Length / sizeof(WCHAR);

#if 0
	// First, check to see if the strings are equal.
	if (RtlEqualUnicodeString(Str, SubStr, TRUE)) {
		return TRUE;
	}
#endif

	// Str and SubStr aren't equal, so now see if SubStr is in Str any where.
	for (index = 0; index + subStrLen <= strLen; ++index) {
		if (_wcsnicmp(Str->Buffer + index, SubStr->Buffer,
			SubStr->Length / sizeof(WCHAR)) == 0) {
			// SubStr is found in Str, so return TRUE.
			return TRUE;
		}
	}
